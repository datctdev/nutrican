...
import com.sba.nutrican_be.ai.MealRecognitionService;
...
public class DietLogService {
    ...
    @Transactional
    public ApiResponse<DietLogResponse> createLog(Long customerId, CreateDietLogRequest request) {
    ...
    @Transactional
    public ApiResponse<AnalyzeMealResponse> analyzeMeal(Long customerId, MultipartFile file, String mealTypeStr) {
        try {
            ...
            com.sba.nutrican_be.core.enums.MealType mealType = null;
            if (mealTypeStr != null) {
                try { mealType = com.sba.nutrican_be.core.enums.MealType.valueOf(mealTypeStr); }
            ...
            return ApiResponse.success(response, aiResult.isFallback() ? "Low confidence, please verify" : "Meal analyzed successfully");
        } catch (Exception e) {
    ...
    public ApiResponse<PageResponse<DietLogResponse>> getLogs(Long customerId, int page, int size,
        ...
        Page<DietLogResponse> mapped = logPage.map(this::toResponse);
        return ApiResponse.success(PageResponse.from(mapped));
    }

    @Transactional(readOnly = true)
    public ApiResponse<DietLogResponse> getLogById(Long logId) {
    ...
    @Transactional
    public ApiResponse<DietLogResponse> updateLog(Long logId, Long userId, CreateDietLogRequest request) {
    ...
    @Transactional
    public ApiResponse<Void> deleteLog(Long logId, Long userId) {
        DietLog dietLog = dietLogRepository.findById(logId)
                .orElseThrow(() -> new ResourceNotFoundException("DietLog", logId));
    ...
    @Transactional(readOnly = true)
    public ApiResponse<DietSummaryResponse> getSummary(Long customerId, LocalDate date) {
        ...
        for (DietLog dietLog : logs) {
            if (dietLog.getMacrosJson() != null) {
                totalCalories = addBigDecimal(totalCalories, getMacro(dietLog, "calories"));
                totalProtein = addBigDecimal(totalProtein, getMacro(dietLog, "protein"));
                totalCarb = addBigDecimal(totalCarb, getMacro(dietLog, "carb"));
                totalFat = addBigDecimal(totalFat, getMacro(dietLog, "fat"));
            }
        }
    ...
    @Transactional
    public ApiResponse<Void> createSosTicket(Long customerId, CreateSosRequest request) {
    ...
    private BigDecimal getMacro(DietLog dietLog, String key) {
        if (dietLog.getMacrosJson() == null || !dietLog.getMacrosJson().containsKey(key)) return BigDecimal.ZERO;
