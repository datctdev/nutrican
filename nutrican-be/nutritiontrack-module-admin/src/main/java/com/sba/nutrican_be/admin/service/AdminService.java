...
public class AdminService {
    ...
    public ApiResponse<PageResponse<User>> getUsers(String role, String status, String search,
                                                   int page, int size) {
    ...
    @Transactional
    public ApiResponse<Void> updateUserStatus(Long userId, String newStatus) {
    ...
    public ApiResponse<PageResponse<PendingPtDto>> getPendingPts(int page, int size) {
        ...
        Page<PendingPtDto> mapped = profilePage.map(this::toPendingPtDto);
        return ApiResponse.success(PageResponse.from(mapped));
    }

    @Transactional
    public ApiResponse<Void> verifyPt(Long userId, PtVerificationRequest request) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("User", userId));
    ...
    public ApiResponse<PageResponse<SOSTicket>> getSosTickets(String status, int page, int size) {
        ...
        if (status != null) {
            ...
            ticketPage = sosTicketRepository.findByStatusIn(
                    java.util.List.of(SOSTicketStatus.OPEN, SOSTicketStatus.ASSIGNED), pageable);
        }
    ...
    @Transactional
    public ApiResponse<Void> assignSosTicket(Long ticketId, Long ptId) {
