...
def estimate_portion_ratio(image: Image.Image) -> dict[str, float]:
    ...
    is_plate = (brightness > 0.82) & (saturation < 0.15)
    is_dark_bg = brightness < 0.12
    is_food = (~is_plate) & (~is_dark_bg) & ((saturation > 0.08) | (brightness < 0.75))

    food_area_ratio = float(is_food.sum()) / total_pixels
    food_area_ratio = max(food_area_ratio, 0.05)

    portion_ratio = food_area_ratio / BASELINE_FOOD_AREA
